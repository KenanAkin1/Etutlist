  ...
  int asilHocaId, string dersGunu, int yedekHocaId, int kacSaat, string telafiNedeni, string aciklama)
     ...
     if (dersProgrami == null)
    {
          dersProgrami = new DersProgrami
             {
        FakulteId = fakulteId, DersAdi = dersAdi, KisimNo = kisimNo,
   HocaId = asilHocaId, DersGunu = dersGunu, DersSaati = dersSaati
          };
    ...
    d.HocaId == yedekHocaId && d.DersGunu == dersGunu && d.DersSaati == dersSaati);
     ...
     string telafiTuru = yedekHocaDersi ? "Birleþtirme" : "Ýkame";
      ...
      {
DersProgramiId = dersProgrami.Id, YedekHocaId = yedekHocaId, FakulteId = fakulteId,
            TelafiTarihi = DateTime.Today, BaslangicSaat = baslangicSaat, BitisSaat = bitisSaat,
             TelafiTuru = telafiTuru, TelafiNedeni = telafiNedeni, Aciklama = aciklama, Onaylandi = false
            };
    ...
    try
 {
     var dersProgrami = await _context.DersProgrami.FirstOrDefaultAsync(d =>
          ...
          if (dersProgrami == null)
      {
       ...
       {
   FakulteId = fakulteId, DersAdi = dersAdi, KisimNo = kisimNo,
         HocaId = hocaId, DersGunu = normalGun, DersSaati = normalDersSaati
           };
  _context.DersProgrami.Add(dersProgrami);
        await _context.SaveChangesAsync();
          }
          ...
          var telafi = new TelafiDers
                {
     DersProgramiId = dersProgrami.Id, YedekHocaId = hocaId, FakulteId = fakulteId,
  TelafiTarihi = telafiTarihi, BaslangicSaat = baslangicSaat, BitisSaat = bitisSaat,
         TelafiTuru = "Telafi", TelafiNedeni = telafiNedeni, Aciklama = aciklama, Onaylandi = false
  };
 ...
 if (!telafiler.Any())
  {
      ...
      var ws = package.Workbook.Worksheets.Add("Ýkame-Birleþtirme Listesi");
            ...
            var headers = new[]
            {
       "S. NU.", "DERS ADI", "SINIF", "BÝRLEÞTÝRÝLEN KISIMLAR", "ÝKÂME EDÝLEN KISIM",
         "DERS SAATÝ", "TOPLAM ÝKAME/BÝRLEÞTÝRME YAPILAN SAAT", "DERS ÖÐRETMENÝ",
     "GÖREVLENDÝRÝLEN ÖÐRETMEN", "DERSÝN NE ÞEKÝLDE ÝÞLENECEÐÝ",
     "ÝKÂME / BÝRLEÞTÝRME SEBEBÝ", "DERSÝN ÝÞLENECEÐÝ YER", "DERSÝN TARÝHÝ"
   };
